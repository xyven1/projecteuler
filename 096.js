...
const prunePossibilities = (puzzleOptions, row, col, val) => {
    ...
    while (deductions) {
      ...
      for (let i = 0; i < 9; i++) {
        for (let j = 0; j < 9; j++) {
          if(puzzleOptions[i][j].length === 0 && puzzle[i][j] === 0)
      ...
      for(let grpIndex = 0; grpIndex < 9; grpIndex++) {
        ...
        for(let digit = 1; digit < 10; digit++) {
          ...
          if(box.filter(v=> v.includes(digit)).length === 1) {5
    ...
    for (let i = 0; i < 9; i++) {
      for (let j = 0; j < 9; j++) {
        ...
        for (let k = 0; k < 3; k++) {
          for (let l = 0; l < 3; l++) {
            if (puzzle[x + k][y + l] === puzzle[i][j] && (x + k !== i || y + l !== j))
              return false
          }
        }
      }
